// Supongamos que pasas el idEncuesta por línea de comandos, por ejemplo:
// node tuScript.js 123

const idEncuesta = process.argv[2]; // 2 = primer argumento después de "node tuScript.js"
if (!idEncuesta) {
  console.error("Debes indicar un id_enquesta, por ejemplo: node tuScript.js 123");
  process.exit(1);
}
...
async function main() {
  ...
  */
    // Obtenemos todas las respuestas
      ...
      'SELECT * FROM respostes WHERE id_enquesta = ?',
      [idEncuesta]
    );
...
for (const filaId in tabla) {
  if (filaId !== "media" && filaId !== "desv") {
    ...
    //const id_enquesta = row.id_enquesta || 1; // ejemplo: 1
    const id_enquesta = idEncuesta; // ejemplo: 1
...
// para que, si ya existe la fila para (id_enquesta, id_alumne), la actualice.
async function volcarEnTablaResultats() {
  for (const r of resultats) {
    // Ajusta según la estructura de tu tabla:
    const sql = `
      INSERT INTO resultats (
        id_enquesta, id_alumne,
        totalAgressivitat,
